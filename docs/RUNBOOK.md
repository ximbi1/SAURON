...
ignored. No tracked credentials found. M5.0-M5.4 are all ACCEPTED against real
...
[HEALTH.md](HEALTH.md)), and Explain 2.0 reusing that health evidence plus
verified-ownership workload→Pod correlation and descriptive metrics (M5.4, see
[EXPLAIN.md](EXPLAIN.md)). `CPU`/`MEM` are default-visible columns; `CPU/R`/
`MEM/R`/`CPU/L`/`MEM/L`/`QOS`/`CPU/C`/`MEM/C`/`CPU/%R`/`MEM/%R`/`CPU/%L`/
`MEM/%L` are wide-only. Next: M5.5 Timeline. See [M5_ACCEPTANCE.md](M5_ACCEPTANCE.md).
